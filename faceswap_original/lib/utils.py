...
class _Backend():  # pylint:disable=too-few-public-methods
    """ Return the backend from config/.faceswap_original of from the `FACESWAP_BACKEND` Environment
    Variable.
    ...
    def _get_config_file(cls):
        ...
        config_file = os.path.join(pypath, "config", ".faceswap_original")
        return config_file

    def _get_backend(self):
        """ Return the backend from either the `FACESWAP_BACKEND` Environment Variable or from
        the :file:`config/.faceswap_original` configuration file. If neither of these exist, prompt the user
        to select a backend.
...
def set_backend(backend):
    ...
    backend: ["amd", "cpu", "nvidia"]
        The backend to set faceswap_original to
    """
