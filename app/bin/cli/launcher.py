...
class ScriptExecutor():  # pylint:disable=too-few-public-methods
    ...
    def _import_script(self):
        ...
        if not pypath.endswith("faceswap"):
            pypath = os.path.join(pypath, "faceswap")
        _config_file = os.path.join(pypath, "config", ".faceswap")
        ...
        if cmd == 'my_faceswap.py':
            # TODO: in config file
            src = "tools.{}".format(self._command.lower()) if self._command in ["alignments"] else "scripts"
        else:
            src = "scripts"
            if cmd == "tools.py":
                src = "tools.{}".format(self._command.lower())

        # if self._command.lower() == 'convert_live':
        #     mod = ".".join((src, 'convert')) #TODO: improve
        # else:
