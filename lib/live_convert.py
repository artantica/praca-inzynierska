#!/usr/bin/env python3
"""
Based on https://github.com/deepfakes/faceswap/blob/master/lib/convert.py
Live Converter for Faceswap """
...
class LiveConverter:
    """ The live converter is responsible for swapping the original face(s) in a frame with the output
    ...
    def __init__(self, output_size, coverage_ratio, arguments, configfile=None):
        logger.debug("Initializing %s: (output_size: %s,  coverage_ratio: %s, "
                     "arguments: %s, configfile: %s)", self.__class__.__name__,
                     output_size, coverage_ratio, arguments,
                     configfile)
    ...
    def reinitialize(self, config):
        """ Reinitialize this :class:`LiveConverter`.
    ...
    def process(self, frame):
        ...
        image = None
        try:
            image = self._patch_image(frame[0])
        except Exception as err:  # pylint: disable=broad-except
            # Log error and output original frame
            logger.error("Failed to convert image. Reason: %s", str(err))
